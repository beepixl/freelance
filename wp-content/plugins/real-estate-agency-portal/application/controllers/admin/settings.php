...
//        exit();
	...
	public function gps_check($str)
	{
        ...
        public function currency_import_from_ecb() {
            ...
            $this->data['currencies_default_id'] = $this->db->get_where('conversions', array('currency_code ='=>'EUR'))->result()[0]->id;
           
            $this->load->library('upload', $config);
                if($this->input->post()) :
                    if(!empty($this->input->post('xml_url'))) {
                    ...
                    if($dom):
                        try {
                        if(!empty($dom->getElementsByTagName('Cube')->item(0))) {
                            $cube_root = $dom->getElementsByTagName('Cube')->item(0);
                        } else {
                            throw new Exception(lang('XML file is not correct'));
                        }  

                        if(!empty($cube_root->getElementsByTagName('Cube')->item(0))) {
                            $cube_root = $cube_root->getElementsByTagName('Cube')->item(0);
                        } else {
                            throw new Exception(lang('XML file is not correct'));
                        }  

                        if(!empty($cube_root->getElementsByTagName('Cube'))) {
                            $cubes =  $cube_root->getElementsByTagName('Cube');
                        } else {
                            throw new Exception(lang('XML file is not correct'));
                        } 
                        ...
                        foreach ($cubes as $key => $value) {
                            $currency='';
                            if(!empty($value->getAttribute('currency')))
                                $currency=$value->getAttribute('currency');
                            $rate='';
                            if(!empty($value->getAttribute('rate')))
                        ...
                        $default_curr=$this->db->get_where('conversions', array('id ='=>$default_curr_id))->result()[0];
