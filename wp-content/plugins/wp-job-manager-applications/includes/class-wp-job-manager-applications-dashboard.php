...
class WP_Job_Manager_Applications_Dashboard {
	...
	public function __construct() {
		...
		add_action( 'wp', array( $this, 'delete_handler' ) );
		add_action( 'wp', array( $this, 'edit_handler' ) );
		add_action( 'wp', array( $this, 'csv_handler' ) );
	...
	public function csv_handler() {
		if ( ! empty( $_GET['download-csv'] ) ) {
			...
			$args = apply_filters( 'job_manager_job_applications_args', array(
				...
				'post_status'         => array( 'publish', 'new', 'interviewed', 'offer', 'hired' ),
