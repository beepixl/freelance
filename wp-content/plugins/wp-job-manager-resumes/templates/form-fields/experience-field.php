<?php if ( ! empty( $field['value'] ) && is_array( $field['value'] ) ) : ?>
	<?php foreach ( $field['value'] as $experience ) : ?>
			...
			<?php foreach ( $field['fields'] as $subkey => $subfield ) : ?>
				<fieldset class="fieldset-<?php esc_attr_e( $subkey ); ?>">
					...
					<div class="field">
						<?php
							// Get name and value
							$subfield['name']  = 'candidate_experience_' . $subkey . '[]';
							$subfield['value'] = $experience[ $subkey ];
...
?>">+ <?php _e( 'Add Experience', 'wp-job-manager-resumes' ); ?></a>
