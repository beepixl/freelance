				...
				<li class="user-resume-package">
					...
					<?php
						if ( $package->get_limit() ) {
							printf( _n( '%s resume posted out of %d', '%s resumes posted out of %s', $package->get_count(), 'wp-resume-manager-wc-paid-listings' ), $package->get_count(), $package->get_limit() );
						} else {
							printf( _n( '%s resume posted', '%s resumes posted', $package->get_count(), 'wp-resume-manager-wc-paid-listings' ), $package->get_count() );
						}
		...
		<?php if ( $packages ) : ?>
			...
			<?php foreach ( $packages as $key => $package ) :
				...
				<li class="resume-package">
					...
					<?php
						printf( _n( '%s to post %d resume', '%s to post %s resumes', $product->get_limit(), 'wp-resume-manager-wc-paid-listings' ) . ' ', $product->get_price_html(), $product->get_limit() ? $product->get_limit() : __( 'unlimited', 'wp-resume-manager-wc-paid-listings' ) );
...
<?php else : ?>
	...
	<p><?php _e( 'No packages found', 'wp-resume-manager-wc-paid-listings' ); ?></p>
